...
class HeightSelectionViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    let infoLabel : UILabel = {
        let label = UILabel()
        label.text = "Height is important in gym applications to calculate body composition metrics and tailor exercise programs accordingly."
        label.translatesAutoresizingMaskIntoConstraints = false
    ...
    let nextButton : UIButton = {
        ...
        btn.addTarget(self, action: #selector(gotoWeightSelection), for: .touchUpInside)
        return btn
    }()
    
    var cameraModePicker: UIPickerView!
    let captureModesList = Array(100...250)
    ...
    @objc func gotoWeightSelection(){
        let controller = WeightSelectionViewController()
